...
export function Waves({
  ...
  const gradientIdRef = useRef(`waves-gradient-${Math.random().toString(36).slice(2, 9)}`)
  ...
  const setLines = () => {
    ...
    if (gradientColors) {
      ...
      gradient.setAttribute('id', gradientIdRef.current)
      ...
      start.setAttribute('stop-color', gradientColors[0])

      const end = document.createElementNS('http://www.w3.org/2000/svg', 'stop')
      end.setAttribute('offset', '100%')
      end.setAttribute('stop-color', gradientColors[1])
    ...
    for (let i = 0; i < totalLines; i++) {
      ...
      path.setAttribute('stroke', gradientColors ? `url(#${gradientIdRef.current})` : strokeColor)
