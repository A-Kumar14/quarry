...
- Session 16: Provenance dots on live trending stories (useSources hook fetches /api/sources → domain map; getProvenanceDot returns colour by state_affiliation/funding_type/editorial_lean/credibility_tier; TrendingPanel renders coloured dot per story; legend row added below list)
- Session 17 (labeled 15 in spec): Pipeline trace accuracy fix (raw_claims_count tracked separately; claims_extracted now counts raw extracted claims not reconciled; claims_verified/claims_contested added to SSE event; pipeline_mode field added: "epistemic" | "pass_through"); pass-through mode "Live query" badge in Source Intelligence panel; empty brief UX guard in CollapsibleAnswer
- Session 18: Deep mode — query decomposition, multi-pass retrieval, sub-query transparency panel, Deep button wired in ExplorePage
- Session 19: WritePage editor overhaul — contentEditable, floating toolbar, keyboard shortcuts, markdown shortcuts, lists, word count, read time, focus mode, citation marker styles; left sidebar collapsible (already was via StorySidebar toggle)
- Session 21: WritePage polish — dark doc card (--doc-surface/--doc-shadow/--doc-fg tokens), dual toolbar (slim fixed bar + enhanced floating with Link/Highlight), APA/MLA moved to export panel as bibliography buttons, drawer handle dark mode fix
